  ...
  buildRSS(channel, items) {
    ...
    if(channel.image && channel.image.length > 1){
      feed.images = channel.image.filter( x=> x.url).map( img => {
        ...
        let im = {};
        if (img.link) im.image.link = img.link;
        if (img.url) im.image.url = img.url;
        if (img.title) im.image.title = img.title;
        if (img.width) im.image.width = img.width;
        if (img.height) im.image.height = img.height;
        return im;
      });
    }
