...
class Parser {
  ...
  buildRSS(channel, items) {
    ...
    // add all channels images to feed, some rss have
    // images in feed before rss item
    if(channel.image && channel.image.length > 1){
      feed.images = channel.image.filter( x=> x.url).map( img => {

        let im = {};
        if (img.link) im.link = img.link[0];
        if (img.url) im.url = img.url[0];
        if (img.title) im.title = img.title[0];
        if (img.width) im.width = img.width[0];
        if (img.height) im.height = img.height[0];
        return im;
      });
    }
  ...
  parseItemRss(xmlItem, itemFields) {
    ...
    if(xmlItem['media:content'] && xmlItem['media:content'].$){
      item.mediaContent = xmlItem['media:content'][0].$;
    }
